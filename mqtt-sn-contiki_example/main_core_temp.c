...
static char     *topics_mqtt[] = {"/temperature", topic_hw};
...
PROCESS_THREAD(init_system_process, ev, data) {
  ...
  while(1) {
      PROCESS_WAIT_EVENT();
      temp_message +=2;
      sprintf(pub_test,"The temperature in the room is: [%d C - degrees]",temp_message-65536);
      mqtt_sn_pub(topic_hw,pub_test,true,0);
      mqtt_sn_pub("/temperature",pub_test,true,0);
